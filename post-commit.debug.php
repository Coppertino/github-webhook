...
#
# Example invocation including payload: 
#http://localhost/github-webhook/post-commit.php?payload={%22ref%22:%22refs/heads/master%22,%22repository%22:{%22url%22:%22https://username:password@github.com/company/repo}}
#
...
if (empty($error) && !empty($_REQUEST['payload'])) {
    ...
    $log.=' is set?'.$payload['repository']['url'].PHP_EOL;
...
if (empty($error) && !empty($sync_conf)) {
  ...
  $log .= '  No sync configuration found'.PHP_EOL;
}


/**
 * STEP 3: 
 * checkout (cache) committed version of master branch 
 */
...
if (empty($error) && !empty($sync_task)) {
	...
	$updated = false;
  $log .= '  About to set repo URL: '.$payload['repository']['url'].PHP_EOL;
  $log .= '  ... to'.str_replace('http://', 'git://', $payload['repository']['url']).'.git'.PHP_EOL;
	$log .= '  ... and cache dir  to'.$cache_dir.PHP_EOL;
	$git = array (
		'$repo_url'	=> str_replace('http://', 'git://', $payload['repository']['url']).'.git',
		'$cache_dir'	=> $cache_dir,
	);
  $log .= '  Repo URL: '.$git['$repo_url'].PHP_EOL;

	# check if cache dir is exists. if not - create it
	if (!is_dir(__CACHE_DIR__)) {
		$log .= ' Created cache dir: '.__CACHE_DIR__.PHP_EOL;
    ...
    $log .= '  Cannot find cache dir: '.($git['$cache_dir']).PHP_EOL;
  } 
  ...
  $log .= '  No sync tasks found'.PHP_EOL;
}
...
if (true) { 
  ...
  echo str_replace(PHP_EOL,'<br>',$log);
}
